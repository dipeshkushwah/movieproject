...
const Header = ({ data }) => {
    ...
    return (
        ...
        >
            ...
            <p
                className='w-[60%] text-white mt-3 mb-3'>
                {data.overview.slice(0, 200)} ...
                <Link to={`/${data.media_type}/details/${data.id}`} className='text-blue-400'>more</Link>
            </p>
            <p className='text-white'>
                <i className="text-yellow-500 mr-2 ri-megaphone-fill"></i>{data.release_date || "No Information"}
                <i className="text-yellow-500 ml-10 mr-2 ri-album-fill"></i>{data.media_type.toUpperCase()}
            </p>
            <Link
